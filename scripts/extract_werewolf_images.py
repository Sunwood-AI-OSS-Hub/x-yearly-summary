#!/usr/bin/env python3
"""
Twitter Archive から AI 人狼関連の画像を抽出するスクリプト
"""
import json
import re
...
OUTPUT_FILE = Path("/prj/x-yearly-summary/docs/ai-werewolf-images.md")

# AI 人狼関連キーワード
WEREWOLF_KEYWORDS = ["AI人狼", "AI.?人狼", "人狼.?AI", "Werewolf", "werewolf", "AIWEV", "$AIWEV"]
...
def is_werewolf_related(text):
    """AI 人狼関連のテキストか判定"""
    text_lower = text.lower()
    for keyword in WEREWOLF_KEYWORDS:
...
def main():
    ...
    for tweet_data in tweets:
        ...
        if not is_werewolf_related(full_text):
            continue

        media_info = extract_media_info(tweet_data)
        if media_info and media_info["media"]:
            werewolf_images.append(media_info)

    print(f"Found {len(werewolf_images)} Werewolf-related tweets with media")

    # Markdown を生成
    md_lines = [
        "# AI 人狼村関連画像",
        "",
        f"**合計**: {len(werewolf_images)} 件のツイート",
        "",
    ]

    for i, img_data in enumerate(werewolf_images, 1):
